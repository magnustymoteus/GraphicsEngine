...
img::EasyImage::EasyImage( int _width,  int _height, Color color) :
...
img::EasyImage& img::EasyImage::operator=(img::EasyImage const& img)
{
 ...
 int img::EasyImage::get_width() const
{
	return width;
}

 int img::EasyImage::get_height() const
{
	return height;
}
...
img::Color& img::EasyImage::operator()( int x,  int y)
{
	assert(x < this->width);
...
img::Color const& img::EasyImage::operator()( int x,  int y) const
{
	assert(x < this->width);
	assert(y < this->height);
	return bitmap.at(x * height + y);
...
void img::EasyImage::draw_line( int x0,  int y0,  int x1,  int y1, Color color)
{
	...
	else
	{
		...
		if (-1.0 <= m && m <= 1.0)
		{
			...
			{
				(*this)(x0 + i, ( int) round(y0 + m * i)) = color;
			}
		}
		else if (m > 1.0)
		{
			...
			{
				(*this)(( int) round(x0 + (i / m)), y0 + i) = color;
			}
		}
		else if (m < -1.0)
		{
...
void img::EasyImage::color_zbuf_pixel( int a,  int i,  int x,  int y, double z0,
                                      double z1, img::Color color, ZBuffer &zbuffer) {
            assert(z0 != 0.0);
            assert(z1 != 0.0);
            double p = (double)i/a;
            double part1 = (1.0f-p)/z0;
            double part2 = p/z1;

            double zval = part1+part2;
            if (zval < zbuffer[y][x]) {
                zbuffer[y][x] = zval;
                (*this)(x,y) = color;
            }
}
void img::EasyImage::draw_zbuf_line( int x0,  int y0, double z0,  int x1,  int y1,
                                     double z1, img::Color color, ZBuffer &zbuffer) {
    if (x0 >= this->width || y0 >= this->height || x1 >= this->width || y1 > this->height) {
        std::stringstream ss;
        ss << "Drawing line from (" << x0 << "," << y0 << ") to (" << x1 << "," << y1 << ") in image of width "
    ...
    if (x0 == x1)
    {
        if(std::max(y0,y1) != y1) std::swap(z0,z1);
        int a = std::max(y0,y1)-std::min(y0,y1);
        //special case for x0 == x1
        for ( int i = std::max(y0, y1); i >= std::min(y0, y1); i--)
        {
            color_zbuf_pixel(a, i-std::min(y0,y1), x0, i, z0, z1, color, zbuffer);
        }
    }
    else if (y0 == y1)
    {
        //special case for y0 == y1
        if(std::max(x0,x1) != x1) std::swap(z0,z1);
        int a = std::max(x0, x1)-std::min(x0, x1);
        for ( int i = std::max(x0, x1); i >= std::min(x0, x1); i--)
        {
            color_zbuf_pixel(a, i-std::min(x0,x1), i, y0, z0, z1, color, zbuffer);
        }
    }
    else
    {
        ...
        if (-1.0 <= m && m <= 1.0)
        {
            ...
            {
                color_zbuf_pixel(a, i, x0+i, (int) round(y0+m*i), z0, z1, color, zbuffer);
            }
        }
        else if (m > 1.0)
        {
            ...
            {
                color_zbuf_pixel(a, i, ( int) round(x0 + (i / m)), y0+i, z0, z1, color, zbuffer);
            }
        }
        else if (m < -1.0)
        {
            int a = y0-y1;
            for ( int i = (y0 - y1); i >= 0; i--)
            {
                color_zbuf_pixel(a, i,(int)round(x0 - (i / m)), y0 - i, z0, z1, color, zbuffer);
            }
        }
...
std::ostream& img::operator<<(std::ostream& out, EasyImage const& image)
{
	...
	//calculate the total size of the pixel data
	 int line_width = image.get_width() * 3; //3 bytes per pixel
	 int line_padding = 0;
	if (line_width % 4 != 0)
	{
	...
	line_width += line_padding;
	 int pixel_size = image.get_height() * line_width;
...
std::istream& img::operator>>(std::istream& in, EasyImage & image)
{
	...
	image.width = std::abs(from_little_endian(header.width));
	 int line_padding = from_little_endian(header.pixel_size) / image.height - (3 * image.width);
