...
namespace
{
	...
	void parse_rules(std::set<char> const& alphabet, std::map<char, std::map<std::string, double>>& rules, stream_parser& parser, bool parse2D)
	{
		parser.skip_comments_and_whitespace();
		...
		while (true)
		{
            ...
            if(c == '{') {
                parser.assertChars("{");
                parser.skip_comments_and_whitespace();
                double probabilities_total = 0.0;
                while(true) {
                    ...
                    rules[alphabet_char].insert({rule, probability});
                    parser.skip_comments_and_whitespace();
                    c = parser.getChar();
                    probabilities_total += probability;
                    if(c == '}') {
                        if(probabilities_total != 100.0f) {
                            throw LParser::ParserException("Expected total probability to equal to 100.0", parser.getLine(), parser.getCol());
                        }
                        break;
                    }
                    else if (c != ',')
                ...
                if (!isValidRule(alphabet, rule, parse2D))
                    throw LParser::ParserException(
                            std::string("Invalid rule specification for entry '") + alphabet_char +
                            "' in rule specification", parser.getLine(), parser.getCol());
                rules[alphabet_char] = {{rule, 100.0f}};
            }
            parser.skip_comments_and_whitespace();
            c = parser.getChar();
			if (c == '}')
...
{
	assert(get_alphabet().find(c) != get_alphabet().end());
	return replacementrules.find(c)->second.begin()->first;
}
std::string LParser::LSystem::get_stochastic_replacement(char c) const
{
    assert(get_alphabet().find(c) != get_alphabet().end());
    std::mt19937 gen(std::random_device{}());
    std::vector<double> chances;
    std::vector<std::string> rules;
    const std::map<std::string, double> &rulesMap = replacementrules.find(c)->second;
    for(auto rule : rulesMap) {
        chances.push_back(rule.second);
        rules.push_back(rule.first);
    }
    std::discrete_distribution<std::size_t> d{chances.begin(), chances.end()};
    std::string random_rule = rules[d(gen)];
    return random_rule;
}
